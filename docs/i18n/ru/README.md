...
- **Проверено вживую, кроме одного чтения.** 25.09.2026 весь процесс прогнан на живом аккаунте, в котором выполнен вход, из приложения Nextbrowser: переключение на вкладку *Following*, чтение ленты, обнаружение нового поста и точное число подписчиков из данных самой страницы. Без входа проверено чтение переписанного фронтенда. Определение автора репоста пока покрыто только фикстурами.
...
- не чаще одного прохода в минуту; пока проход не закончился, следующий не начнётся;
- каждый профиль читается не больше одного раза за проход;
